...
public class StarAdapter extends MultiTypeAdapter {
    private final static String DYNAMIC_TAG = "dynamic";
    private final static String TOPICS_TAG = "topics";
    private final static String EMPTY = "empty";
    private final static int DYNAMIC = 0;

    private final static int TAG = 0;
    private final static int DYNAMIC_CREATE_TIME = 1;
    private final static int DYNAMIC_DESC = 2;
    private final static int DYNAMIC_COMMENT_COUNT = 3;
    private final static int DYNAMIC_LIKE_COUNT = 4;
    private final static int DYNAMIC_DESC_IMAGE_1 = 5;
    private final static int DYNAMIC_DESC_IMAGE_2 = 6;
    private final static int DYNAMIC_DESC_IMAGE_LAYOUT = 7;

    // 话题
    private final static int ROOT = 8;
    private final static int AVATAR_LAYOUT = 9;
    private final static int AVATAR = 10;
    private final static int NICKNAME = 11;
    private final static int CREATE_TIME = 12;
    private final static int DESC = 13;
    private final static int DESC_IMAGE_LAYOUT = 14;
    private final static int DESC_IMAGE = 15;
    private final static int SUPPORT_COUNT = 16;
    private final static int POST_LAYOUT = 17;
    private final static int CONTAINER_LAYOUT = 18;
    private final static int SUPPORT_LAYOUT = 19;
    private final static int GRID = 20;
    private final static int SHARE = 21;
    private final static int SUPPORT = 22;
    private final static int POST = 23;
    private final static int SUPPORT_TEXT = 24;
    private final static int DEFAULT_DELETE_BUTTON = 25;
    private final static int DELETE_LAYOUT = 26;
    private final static int PRIZE_LAYOUT = 27;
    private final static int PRIZE_TEXT = 28;
    private final static int BADGE_LAYOUT = 29;
    private final static int PRIZE_AND_SUPPORT_AND_POST = 30;
    private final static int DIVIDER = 31;

    private final static int DYNAMIC_INCLUDE = 32;

    private final static int RULE_INCLUDE = 33;
    private final static int RULE_TEXT = 34;

    private final static int DESC_IMAGE_PROSPECT = 35;

    private final static int EMPTY_LAYOUT = 36;
    private final static int EMPTY_TEXT = 37;

    private final StarActivity activity;
    ...
    public StarAdapter(StarActivity activity) {
        super(activity);
    ...
    protected int[] getChildViewIds(int type) {
        return new int[]{
                //动态
                R.id.star_dynamic_tag,
                R.id.star_dynamic_createtime,
                R.id.star_dynamic_desc,
                R.id.star_dynamic_comment_count,
                R.id.star_dynamic_like_count,
                R.id.star_desc_image_1,
                R.id.star_desc_image_2,
                R.id.star_desc_image_layout,
                ...
                //所有layout的root
                R.id.star_dynamic_include,
                R.id.star_topics_rule_layout,

                //版规
                R.id.star_topics_rule,

                //动态图片前景
                R.id.star_dynamic_desc_image_prospect,
    ...
    protected void update(int position, Object item, int type) {
        if (DYNAMIC_TAG.equals(tag)) {
            setDynamicData((Topic) item);
        } else if (TOPICS_TAG.equals(tag)) {
            setTopicData((Topic) item, position);
        } else if (EMPTY.equals(tag)) {
            setText(EMPTY_TEXT, ((Topic) item).getMessage());
            setGone(EMPTY_LAYOUT, false);
            setGone(DYNAMIC_INCLUDE, true);
            setGone(ROOT, true);
        }
    }

    private void setTopicData(final Topic item, final int position) {
        ...
        if (item.getIsTop() == 1) {
            //1为显示版规, 0为不显示
            setGone(RULE_INCLUDE, false);
            setGone(ROOT, true);
            FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, Utils.dip2px(activity, 48));
            params.topMargin = Utils.dip2px(activity, 6);
            view(RULE_INCLUDE).setLayoutParams(params);

            setText(RULE_TEXT, item.getTitle());
        } else {
            setGone(RULE_INCLUDE, true);
            setGone(ROOT, false);
        }
...
//                        topic.setIsFavorit(1);
    ...
    private void setDynamicData(Topic topics) {
        setGone(RULE_INCLUDE, true);
        setGone(DYNAMIC_INCLUDE, false);
        setGone(ROOT, true);
        setGone(EMPTY_LAYOUT, true);
        if (topics != null) {
            setText(DYNAMIC_CREATE_TIME, topics.getCreateTime());
            if (TextUtils.isEmpty(topics.getMessage())) {
                setGone(DYNAMIC_DESC, true);
            } else {
                setGone(DYNAMIC_DESC, false);
                setText(DYNAMIC_DESC, topics.getMessage());
            }
            setNumber(DYNAMIC_COMMENT_COUNT, topics.getPostsCnt());
            setNumber(DYNAMIC_LIKE_COUNT, topics.getFavoritsCnt());
            if (topics.getPics() != null && !topics.getPics().isEmpty()) {
                setGone(DYNAMIC_DESC_IMAGE_LAYOUT, false);
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(width, width / 4 * 3);
                imageView(DYNAMIC_DESC_IMAGE_2).setLayoutParams(params);
                imageView(DESC_IMAGE_PROSPECT).setLayoutParams(params);

                RelativeLayout.LayoutParams imageParams = new RelativeLayout.LayoutParams(width - Utils.dip2px(activity, 40), width / 4 * 3 + Utils.dip2px(activity, 8));
                imageParams.addRule(RelativeLayout.CENTER_IN_PARENT);
                view(DYNAMIC_DESC_IMAGE_1).setLayoutParams(imageParams);

                imageView(TAG).setImageResource(R.drawable.icon_feedimg_tomato);
                if (topics.getPics().size() > 1) {
                    ImageLoader.getInstance().displayImage(topics.getPics().get(0).getImg(), imageView(DYNAMIC_DESC_IMAGE_1), ImageUtils.imageLoader(activity, 0));
                    ImageLoader.getInstance().displayImage(topics.getPics().get(1).getImg(), imageView(DYNAMIC_DESC_IMAGE_2), ImageUtils.imageLoader(activity, 0));
                } else {
                    setGone(DYNAMIC_DESC_IMAGE_1, true);
                    ImageLoader.getInstance().displayImage(topics.getPics().get(0).getImg(), imageView(DYNAMIC_DESC_IMAGE_2), ImageUtils.imageLoader(activity, 0));
                }
            } else {
                setGone(DYNAMIC_DESC_IMAGE_LAYOUT, true);
                imageView(TAG).setImageResource(R.drawable.icon_feedspeak_tomato);
