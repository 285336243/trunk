 ...
 * 广告的adapter
 */
public class AdverFeedAdapter extends FeedAdapter {
     ...
     * @param fragment
     */
    public AdverFeedAdapter(FeedItemFragment fragment) {
        super(fragment);
        initAd();
        initView();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mMobiSageAdNativeFactory.getAdNatives().size() == 0 || type == 1) {
            ...
            Log.v("kan","返回广告 position = "+position+", size = "+mMobiSageAdNativeFactory.getAdNatives().size()+", type = "+type);
...
//        options.round = (int) (5 * activity.getResources().getDisplayMetrics().density);
...
//        ad.id(R.id.native_sample_item_lgo).image(
//                (String) hash.get("logo"), options);
//        ad.id(R.id.native_sample_item_image).image(
//                (String) hash.get("image"), true, true, 0,
//                0, null, AQuery.FADE_IN, AQuery.RATIO_PRESERVE);
...
//        ad.id(R.id.native_sample_item_title).text((String)
//                hash.get("title"));
...
//        ad.id(R.id.native_sample_item_people).text(
//                "有" + (Integer) hash.get("person") + "人正在玩");
