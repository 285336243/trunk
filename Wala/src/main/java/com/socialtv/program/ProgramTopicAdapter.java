 ...
 * 节目组中话题的Adapter
  */
 public class ProgramTopicAdapter extends MultiTypeAdapter {
    ...
    private final static int PRIZE_AND_SUPPORT_AND_POST = 22;
     ...
     private final static int EMPTY_LAYOUT = 23;
     private final static int EMPTY_TEXT = 24;
     private final static int RULE_LAYOUT = 25;
     private final static int RULE_TEXT = 26;
     private final static int DIVIDER = 27;
     ...
     private final ProgramTopicFragment fragment;
      ...
      */
     @Inject
     public ProgramTopicAdapter(final ProgramTopicFragment fragment) {
     ...
     public void addItems(List<Topic> items, String tag) {
         this.tag = tag;
         addItems(0, items);
     }

     public void setItems(List<Topic> items, String tag) {
         this.tag = tag;
         clear();
         addItems(0, items);
     }
     ...
     protected int[] getChildViewIds(int type) {
         return new int[] {
                 ...
                 R.id.program_item_rule_layout,
                 R.id.program_item_rule,
                 R.id.item_divider
          };
     }
     ...
     protected void update(int position, Object item, int type) {
         try {
             if (item != null) {
                 ...
                 if (EMPTY.equals(tag)) {
                     setGone(ROOT, true);
                     setGone(RULE_LAYOUT, true);
                     setGone(EMPTY_LAYOUT, false);
                     setText(EMPTY_TEXT, topic.getMessage());
                 } else {
                     ...
                     if (topic != null) {
                         if (topic.getIsTop() == 1) {
                             setGone(ROOT, true);
                             setGone(RULE_LAYOUT, false);
                             setText(RULE_TEXT, topic.getTitle());
                             view(RULE_LAYOUT).setOnClickListener(new View.OnClickListener() {
                                 @Override
                                 public void onClick(View v) {
                                     activity.startActivity(new Intents(activity, TopicDetailActivity.class).add(IConstant.TOPIC_ID, topic.getId()).toIntent());
                                 }
                             });
                         } else {
                             setGone(ROOT, false);
     ...
     private void addPostToView(final Topic item, final int count) {
         ...
         if (item.getPostsCnt() > 10) {
             ...
             v.setOnClickListener(new View.OnClickListener() {
                 @Override
                 public void onClick(View view) {
                     fragment.startActivityForResult(new Intents(activity, TopicDetailActivity.class).add(IConstant.TOPIC_ID, item.getId()).toIntent(), 0);
                 }
             });
         }
     }
