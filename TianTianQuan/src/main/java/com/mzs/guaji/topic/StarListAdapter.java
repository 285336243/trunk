...
public class StarListAdapter extends MultiTypeAdapter {
    ...
    private void addItem(String string) {
        addItem(NO_STAR_DYNAMIC_INFORMATION, string);

    }

    private void addItem(Topic topic) {
        addItem(STAR_TOPIC, topic);
    ...
    protected int[] getChildViewIds(int type) {
        return new int[]{
                ...
                R.id.find_post_cnt, //评论数量 17
                R.id.have_information_layout, //有信息的layout 18
                R.id.no_information_layout, //没有信息的layout  19
                R.id.empty_information_text,//没有信息的显示textview 20
                R.id.total_layout, //有信息的总的  layout 21
                R.id.like_comment_layout, //喜欢评论       layout 22
                R.id.space_view,    //       间隔view  23
                R.id.find_post_cnt,  //话题评论数量 24
                R.id.find_like_cnt //话题赞的数量  25
        };
    }
    ...
    protected void update(int position, Object item, int type) {
        ...
        if (item instanceof Topic) {
            switch (type) {
                case STAR_TOPIC:
                    setTopicContent((Topic) item);
            }
        }
        if (item instanceof String) {
            switch (type) {
                case NO_STAR_DYNAMIC_INFORMATION:
