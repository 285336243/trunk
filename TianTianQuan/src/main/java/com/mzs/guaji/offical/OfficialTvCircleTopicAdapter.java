 ...
 */
public class OfficialTvCircleTopicAdapter extends SingleTypeAdapter<Topic> {
...
//    @Inject
     ...
     * @param activity
     */
    public OfficialTvCircleTopicAdapter(Activity activity) {
        super(activity, R.layout.find_topic);
        this.activity = activity;
        this.imageLoader = ImageLoader.getInstance();
        service = new OfficialTopicService(activity);
     ...
     */
    @Override
    protected void update(int position, Topic item) {
        if (item != null) {
            setContent(item);
        }
    }

    private void setContent(final Topic topicItem) {
