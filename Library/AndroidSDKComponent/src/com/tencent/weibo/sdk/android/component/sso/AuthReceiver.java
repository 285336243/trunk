...
public class AuthReceiver extends BroadcastReceiver {
		...
		if (intent.getAction().equals(ACTION)) {
			...
			if (packageName.equals(context.getPackageName())) { // 是这个app发起的授权请求
				...
				if (authResult) {
                    ...
                    boolean isWeb = intent.getBooleanExtra("com.tencent.sso.WEB", false);
                    if(isWeb) {
                        if (AuthHelper.listener != null) {
                            AuthHelper.listener.onAuthPassed(name, getBindingInfo(context));
                        }
                    }else {
                        byte[] data = intent
    ...
    private WeiboToken getBindingInfo(Context context) {
        WeiboToken token = new WeiboToken();
        token.accessToken = Util.getSharePersistent(context, "ACCESS_TOKEN");
        token.expiresIn = Long.parseLong(Util.getSharePersistent(context, "EXPIRES_IN"));
        token.openID = Util.getSharePersistent(context, "OPEN_ID");
        token.omasKey = Util.getSharePersistent(context, "OPEN_KEY");
        token.refreshToken = Util.getSharePersistent(context, "REFRESH_TOKEN");
        return token;
    }
