package com.shengzhish.xyj.activity.precioushouse.ibeancon;
...
public class BeaconDetactorService extends Service implements IBeaconConsumer {
	...
	private static void generateNotification(Context context, String message) {
		...
		Intent launchIntent = new Intent(context, PreciousActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
				| Intent.FLAG_ACTIVITY_SINGLE_TOP);

		((NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE)).notify(
				0,
				new NotificationCompat.Builder(context).setWhen(System.currentTimeMillis())
						.setSmallIcon(R.drawable.icon).setTicker(message)
