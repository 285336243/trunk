 ...
 * 动态里的最新和热门的Adapter
 */
public class FeedAdapter extends SingleTypeAdapter<Topic> {
    ...
    private final static int EMPTY_LAYOUT = 22;
    private final static int EMPTY_TEXT = 23;

    private final FeedItemFragment fragment;
     ...
     */
    public FeedAdapter(final FeedItemFragment fragment) {
        super(fragment.getActivity(), R.layout.feed_item);
     ...
     */
    @Override
    protected void update(int position, final Topic item) {
        try {
            if (item != null) {
                if (EMPTY.equals(tag)) {
                    //空的,显示默认的
    ...
    private void addPostToView(final Topic item, final int count) {
        ...
        if (item.getPostsCnt() > 10) {
            ...
            v.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    fragment.getParentFragment().startActivityForResult(new Intents(activity, TopicDetailActivity.class).add(IConstant.TOPIC_ID, item.getId()).toIntent(), 2);
                }
            });
        }
    }
