...
public class StorageUtil {
	...
	public static File makeCacheDir(String saveDir) {
		if(isExternalStorageAvailable()) {
			File savedir = new File(Environment.getExternalStorageDirectory(),"/GuaJi/"+saveDir);
			if (!savedir.exists()) {
				savedir.mkdirs();
			}
			return savedir;
    ...
    public static File getFilePath(String path) {
        if(isExternalStorageAvailable()) {
            File file =  new File(Environment.getExternalStorageDirectory(),"/GuaJi/"+path);
            if(file.exists()) {
                return file;
            }else {
                return null;
    ...
    public static File makeFile(String fileName) {
        if(isExternalStorageAvailable()) {
            File file = new File(Environment.getExternalStorageDirectory(),"/GuaJi/"+fileName);
            if(!file.exists()) {
                try {
                    file.createNewFile();
                    return file;
    ...
    public static String getSDCardPath() {
        if(isExternalStorageAvailable()) {
            return Environment.getExternalStorageDirectory()+"/GuaJi/";
        }
        return "";
    }
