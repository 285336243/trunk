...
public class HttpMultipartPostTest extends AsyncTask<String, Integer, String> {
    ...
    public HttpMultipartPostTest(Activity context, String uri, File filePath, Map<String, String> paramap) {
...
//		HttpPost httpPost = new HttpPost("上传URL， 如：http://www.xx.com/upload.php");
		...
		try {
			...
			multipartContent.addPart("input_name", new FileBody(filePath));
//            multipartContent.addPart("orderid",new StringBody(map.get("orderid")));
            multipartContent.addPart("username",new StringBody(map.get("username")));
