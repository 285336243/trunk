...
//    private GuaJiAPI mApi = GuaJiAPI.newInstance(this);
...
//            	ToastUtil.showToast(context, "正在同步资料");
    ...
    private void createGuaJiAccount(OAuthToken oAuthToken, String type) {
        ...
        new AbstractRoboAsyncTask<ExternalSignin>(context){
            @Override
            protected ExternalSignin run(Object data) throws Exception {
                SynchronizationHttpRequest request = RequestUtils.getInstance().createPost(context, DOMAIN+"identity/externalsignin.json", param, null);
                request.setType(new TypeToken<ExternalSignin>(){}.getType());
                return (ExternalSignin) request.getResponse();
            }

            @Override
            protected void onSuccess(ExternalSignin response) throws Exception {
...
//        }, null);
    ...
    private void postSinaBindingAccount(final String type, String uid, String token, String expiredTime) {
        ...
        new AbstractRoboAsyncTask<BindingAccount>(context){
            @Override
            protected BindingAccount run(Object data) throws Exception {
                SynchronizationHttpRequest request = RequestUtils.getInstance().createPost(context, getBindingAccountRequest(), headers, null);
                request.setType(new TypeToken<BindingAccount>(){}.getType());
                return (BindingAccount) request.getResponse();
            }

            @Override
            protected void onSuccess(BindingAccount response) throws Exception {
                super.onSuccess(response);
                if(response != null) {
                    if(response.getResponseCode() == 0) {
                        if("SINA".equals(type)) {
